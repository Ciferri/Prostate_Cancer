...
int diffusion3D::initModel(int x, int y, int z, double pO2)
{
  ...
  ((prostateCell *)(m_comp->at(indice)))->initModel(0.0, 0.0, 0.0, 1.0, pO2); 	/**< Pour un vaisseau, on met la pO2 à 42mmHg */
...
//~ int diffusion3D::initModel(double *tab,int size_x, int size_y)
//~ {
//~ int count= 0;
...
//~ {
//~ //if(valeur de l'image == rouge) { (m_comp->at(indice))->ModelRAZ(0.0, 0.0, 0.0, 1.0, pO2); }
//~ //else if(valeur de limage == bleu) { (m_comp->at(indice))->ModelRAZ(0.0, 0.0, 1.0, 0.0, pO2); }
//~ //etc
//~ count += 1;
//~ }
//~ }
...
int diffusion3D::updateModel(double time, const double DT)
{
  ...
  for (int k=0;k<m_nlayer;k++)
	...
	//~ ((prostateCell*)tissue[k][i][j])->setVmax(m_Vmax);
	//~ ((prostateCell*)tissue[k][i][j])->setKm(m_K_conso);
  ...
  /*
   * Représentation d'un cube avec 6 face (nord,sud est, ouest, seuil et top)
   * */
  ///////////////////
  //  Sommets‎ (8)  //
  ///////////////////
   ...
   * */
  //~ int *diffusion3D::Coord_K_to_XY(int k)
  //~ {
  //~ int *tab= new int[2];
  //~ tab[0] = k/m_ncol;
  //~ tab[1] = k%m_ncol;
